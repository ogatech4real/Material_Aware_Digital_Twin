...
"""
Pareto Frontier Analysis: Annual Cost vs Battery Wear (EFCs)
------------------------------------------------------------
Sweeps λ_batt for the current core controller, computes KPIs, extracts the true Pareto frontier,
fits a smooth curve, detects the knee, and saves both CSV + plot.
...
def bootstrap_daily_cost(df_dispatch: pd.DataFrame, dt_h: float, n: int = 1000, seed: int = 7) -> Tuple[float, float, float]:
    """
    Optional helper: bootstrap 95% CI on mean daily cost.
    ...
    if isinstance(df_dispatch.index, pd.DatetimeIndex):
        ...
        # Fallback: assume contiguous 15-min steps → 96 steps/day
...
def _extract_frontier(df: pd.DataFrame) -> pd.DataFrame:
    """
    Build the Pareto frontier (lower envelope) by sorting by EFC ascending
    and retaining points that strictly improve (reduce) cost.
    """
...
def _fit_frontier(frontier: pd.DataFrame, n_eval: int = 200) -> Tuple[np.ndarray, np.ndarray]:
    """
    Quadratic fit (cost vs EFC) on the frontier for a smooth publishable curve.
    Fit on normalized x for numerical conditioning; return dense x,y for plotting.
    """
    x = frontier["equivalent_full_cycles"].values
    y = frontier["annual_cost_gbp"].values
...
def run_pareto_sweep(
    ...
    """
    Perform a λ_batt sweep for the current core controller, compute KPIs,
    save CSV and plot a publishable Pareto curve with knee.

    NOTE: lambda_pv is not swept because the frozen core controller does not
    contain a controllable PV-degradation pathway; varying it previously had no
    effect on dispatch and produced duplicate points.
    """
    _ensure_dirs()
    ...
    # Verified correction: lambda_pv is diagnostic in the present core model and
    # does not influence dispatch. Keep a single reference value to avoid an
    # artificial two-dimensional Pareto sweep.
    lam_pv_grid = np.array([float(conf.get("economics", {}).get("lambda_pv", 0.0))])

    rows = []
    e_nom = conf["battery"]["e_nom_kwh"]
    print("Running Pareto sweep for λ_batt (lambda_pv held fixed; core PV path is exogenous)...")
    for lb in lam_b_grid:
        for lp in lam_pv_grid:
            ...
            print(f"  λ_batt={lb:.2f}, λ_pv={lp:.2f} → Cost £{kpi['annual_cost_gbp']:.1f}, EFC {kpi['equivalent_full_cycles']:.1f}")
    ...
    # ---- Plot: scatter, lower envelope, quadratic fit, knee ----
    _plot_pareto(pareto)
    return pareto


def _plot_pareto(pareto_df: pd.DataFrame):
    ...
    sc = plt.scatter(
        ...
        cmap="viridis",
        s=60, edgecolor="k", linewidths=0.5, alpha=0.9,
        label="Grid (λ_batt × λ_pv)"
    )
    plt.plot(frontier["equivalent_full_cycles"], frontier["annual_cost_gbp"],
             linestyle="--", color="black", linewidth=1.2, label="Frontier (lower envelope)")
    if len(x_fit) > 2:
        plt.plot(x_fit, y_fit, color="black", linewidth=1.8, alpha=0.85, label="Quadratic fit")
    plt.scatter([xk], [yk], s=110, marker="D", color="crimson", edgecolor="white", linewidths=1.0, zorder=5, label="Knee")
...
def _load_conf(path: str = "config.yaml") -> dict:
    with open(path, "r") as f:
        return pd.io.json._json.loads(f.read()) if path.endswith(".json") else __import__("yaml").safe_load(open(path))
...
if __name__ == "__main__":
    ...
    #   python -m src.analysis_extensions
    import yaml
    try:
